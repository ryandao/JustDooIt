...
/**
 * Action that marks a task as done.
 * 
 */
public class DoneAction extends UIAction {
	 ...
	 *            The user file.
	 */
	public DoneAction(GraphicalInterface ui, Logic logic, String userInput) {
	...
	public void markTaskDone(int taskId, boolean done){
		try{
			...
			if(task.getStatus() == true){			
				message = CLILogic.MESSAGE_ALREADY_DONE.replace("%1s", Integer.toString(taskId));
				_UI.showFeedBackMessage(message);
			}
			else{
				_LOGIC.setStatus(taskId, done);
				message = CLILogic.MESSAGE_DONE.replace("%1s", Integer.toString(taskId));				
